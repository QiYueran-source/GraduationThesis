...
class DatabaseLoader:
    ...
    def load_data_handler(self, message: Message):
        ...
        for each_year in year_list:
            ...
            factors_df = get_factors_data(code_list, factors_start_date, factors_end_date).with_columns(
                pl.col('accper').dt.strftime('%Y-%m-%d').alias('accper')
            )
            return_df = get_stock_return(code_list, return_start_date, return_end_date).with_columns(
                pl.col('accper').dt.strftime('%Y-%m-%d').alias('accper')
            )
